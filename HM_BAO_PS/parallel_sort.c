...
void parallel_sort(void *base, size_t nmemb, size_t size, int (*compar) (const void *, const void *))
{
  ...
  if(Local_NTask > 1 && Color == 1)
	...
	noffs[i] = noffs[i - 1] + nlist[i - 1];
      ...
      char *element_guess = mymalloc("element_guess", (Local_NTask - 1) * size);
      size_t *element_tie_braking_rank =
	mymalloc("element_tie_braking_rank", (Local_NTask - 1) * sizeof(size_t));
      size_t *desired_glob_rank = mymalloc("desired_glob_rank", (Local_NTask - 1) * sizeof(size_t));
      size_t *current_glob_rank = mymalloc("current_glob_rank", (Local_NTask - 1) * sizeof(size_t));
      size_t *current_loc_rank = mymalloc("current_loc_rank", (Local_NTask - 1) * sizeof(size_t));
      long long *range_left = mymalloc("range_left", (Local_NTask - 1) * sizeof(long long));
      long long *range_right = mymalloc("range_right", (Local_NTask - 1) * sizeof(long long));
      int *max_loc = mymalloc("max_loc", (Local_NTask - 1) * sizeof(int));
      size_t max_nmemb;
      ...
      do
	{
      ...
      size_t *Send_count = mymalloc("Send_count", Local_NTask * sizeof(size_t));
      size_t *Recv_count = mymalloc("Recv_count", Local_NTask * sizeof(size_t));
      size_t *Send_offset = mymalloc("Send_offset", Local_NTask * sizeof(size_t));
      size_t *Recv_offset = mymalloc("Recv_offset", Local_NTask * sizeof(size_t));
...
void serial_sort(char *base, size_t nmemb, size_t size, int (*compar) (const void *, const void *))
{
			      ...
			      int (*compar) (const void *, const void *))
{
  ...
  nlist = (size_t *) mymalloc("nlist", NTasks * sizeof(size_t));
  ...
  for(i = ThisTask + 1, send = -1; nmemb > 0 && i < NTasks; i++)
    if(nlist[i] > 0)
      {
	send = i;
	break;
