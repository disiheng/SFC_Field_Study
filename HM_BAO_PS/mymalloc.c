...
void report_detailed_memory_usage_of_largest_task(size_t * OldHighMarkBytes, const char *label, const char *func, const char *file, int line)
{
  ...
  sizelist = (size_t *) mymalloc("sizelist", NTasks * sizeof(size_t));
  MPI_Allgather(&AllocatedBytes, sizeof(size_t), MPI_BYTE, sizelist, sizeof(size_t), MPI_BYTE, MPI_COMM_WORLD);

  for(i = 1, task = 0, maxsize = minsize = sizelist[0], avgsize = (double) sizelist[0]; i < NTasks; i++)
    {
  ...
  if(Nblocks >= MAXBLOCKS)
    {
      ...
      sprintf(buf, "Task=%d: No blocks left in mymalloc_fullinfo() at %s()/%s/line %d. MAXBLOCKS=%d", ThisTask, func, file, line, MAXBLOCKS);
      terminate(buf);
    }
  ...
  if(nr < 0)
    {
      ...
      sprintf
	(buf, "Task=%d: Wrong call of myrealloc_movable() from %s()/%s/line %d - this block has not been allocated!\n", ThisTask, func, file, line);
      terminate(buf);
    }
